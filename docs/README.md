...
- **No hardcoded tenant slug or domain** – Use env: `NUXT_PUBLIC_DEV_TENANT`, `NUXT_PUBLIC_TENANT_BASE_DOMAIN`, `VERIFY_URL_TEMPLATE`, etc. API seeds all tenants from registry (no single slug in code). See [Environments](../.cursor/memory/environments.md).
...
| Doc | Purpose |
|-----|---------|
| [Adding a new module](../.cursor/memory/adding-a-new-module.md) | End-to-end guide: scope, catalog, nav, pricing, API, Admin, activation (agent memory) |
| [Environments](../.cursor/memory/environments.md) | Env vars by app, run locally, deploy (API/Tenant/Platform), migrations, Discord reference |
| [DB restore](../.cursor/memory/db-restore.md) | Restore Postgres from R2 backup (disaster recovery, migration) |
